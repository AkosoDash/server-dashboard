...
// Standalone daily report script - doesn't depend on Next.js build
...
async function queryPrometheusRange(query, minutes = 60) {
  try {
...
async function queryPrometheusAll(query) {
  try {
    const res = await fetch(
      `${PROMETHEUS_URL}/api/v1/query?query=${encodeURIComponent(query)}`,
      { next: { revalidate: 10 } },
