...
interface Metrics {
  ...
  network: {
    rx: number | null;
    tx: number | null;
    rxHistory: { time: string; value: number }[];
    txHistory: { time: string; value: number }[];
  };
  disks: {
    device: string;
    mountpoint: string;
...
export default function Dashboard() {
  ...
  const cpuColor =
    (metrics?.cpu.value ?? 0) > 80
      ? "#ff4466"
      : (metrics?.cpu.value ?? 0) > 60
        ? "#ffcc00"
        : "#00d4ff";
  const ramColor =
    (metrics?.ram.percent ?? 0) > 80
      ? "#ff4466"
      : (metrics?.ram.percent ?? 0) > 60
        ? "#ffcc00"
        : "#00ff88";
      ...
      >
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            />
            <span
              className="font-mono font-semibold tracking-widest text-sm uppercase"
              style={{ color: "#00d4ff" }}
            >
        ...
        {loading ? (
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <div className="text-xs font-mono" style={{ color: "#4a6375" }}>
                opening stream...
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <StatCard
                ...
                value={
                  metrics?.network.rx
                    ? formatNetworkRate(metrics.network.rx)
                    : "—"
                }
              ...
              <StatCard
                label="Containers"
                value={String(metrics?.containers.length ?? "—")}
                unit="running"
                color="accent"
                index={3}
              />
